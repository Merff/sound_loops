...
**Baseline** (30 лупов, `evals/runs/20260920T125514_56840b8.json`; переснят
после замены части лупов и расширения `good_tracks` в разметке — числа до
этого прогона несравнимы с текущими, см. `git log -- evals/dataset.json`):
...
**Сравнение конфигураций гибридного поиска** (30 лупов, `eval_search_depth=500`,
метрика — `mean_penalized_rank`; `evals/runs/20260920T131920_56840b8.json` —
...
Фильтры и реранк не улучшают retrieval на текущем промпте шага B — отсюда
рекомендация не включать `--filters`/`--rerank` по умолчанию. Реранк
архитектурно оправдан (читаемые объяснения, чистый RAG), но пользы на
этом наборе не показал.

**Надёжность вызова инструмента агентом** (30 лупов,
`eval-run --agent`, `evals/runs/20260920T132553_56840b8.json`):
...
`good_tracks` (теперь по 2–3 реальных трека на луп вместо одного) сняли
эффект, ради которого диверсификация запросов раньше выглядела сильно
...
скудности старой разметки; надо перепроверять на большем датасете, а не
