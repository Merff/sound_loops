...
Гибридный поиск с SQL-фильтрами по темпу/вокалу и лестницей послаблений
существовал до этого отдельной конфигурацией и удалён: на 30 лупах он не
улучшил ни одной метрики, а лестница ни разу не срабатывала (числа —
ниже). Фильтры по темпу и вокалу остались там, где ими управляет модель —
в аргументах инструмента `search_music` у агента.
...
### Почему удалён гибридный поиск

30 лупов, `eval_search_depth=500`, метрика — `mean penalized rank` (меньше
...
Фильтры не улучшили retrieval (penalized rank не ниже, число «не найдено»
даже выросло), а лестница послаблений за все 30 лупов не сработала ни разу
— то есть обрабатывала сценарий, которого на этих данных не существует.
Обе вещи удалены вместе с флагом `--filters`; прогоны выше оставлены как
запись измерения. Реранк архитектурно оправдан (читаемые объяснения,
чистый RAG) и остался, но пользы на этом наборе тоже не показал — и по
построению не может: он переставляет только первые три позиции, поэтому
`hit@5` у него тождественно равен неранжированной конфигурации.
