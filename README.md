...
CLAP). Живёт в графе-агенте отдельным узлом. Отдельной конфигурацией
эвала переранжирование не меряется: оно переставляет только первые три
позиции, поэтому `hit@5` у него тождественно равен конфигурации без
него — измерять оно способно один `hit@1` (см. «Эвалы» ниже).
...
- **hit@1 / hit@5** — доля лупов, где хотя бы один из размеченных
  ...
  Выше — лучше.
- **mean penalized rank** — на каком месте в выдаче поиска оказался лучший
  из `good_tracks`, в среднем по лупам. Лупы «не найдено» из среднего не
  выбрасываются, а получают штраф, равный `eval_search_depth` (500), как
  будто трек нашёлся на самом дне выдачи: без штрафа конфигурация, которая
  чаще вообще проваливает поиск, показывала бы средний ранг лучше — просто
  за счёт того, что выбрасывает из среднего самые сложные случаи. Меньше —
  лучше, 1 — трек первым в выдаче у каждого лупа.
...
| mean penalized rank | 259.7 (не найдено в топ-500: 11/30) |
