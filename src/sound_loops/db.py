...
Схема больше не собирается из одного SQL-скрипта с ручными
IF NOT EXISTS/DO-блоками — каждое изменение схемы оформляется отдельным
пронумерованным файлом в migrations/, yoyo сам ведёт учёт применённых
миграций и гарантирует, что каждая накатится ровно один раз.
"""
