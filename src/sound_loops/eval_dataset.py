"""Разметка набора лупов для эвала. Хранится одним файлом в git (evals/dataset.json).
Разметка делается руками (прослушивание/просмотр), этот модуль только валидирует формат
и даёт понятную ошибку на битую метку.
"""
