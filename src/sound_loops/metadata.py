"""Лучшее-по-возможности чтение метаданных треков из tracks.csv датасета FMA.
...
Разбор нужен только для трёх полей и не критичен для итерации 0: если
формат отличается от ожидаемого, метаданные остаются пустыми, это не
ошибка — вызывающий код продолжает работу.
"""
...
def load_metadata(csv_path: Path) -> dict[int, TrackMetadata]:
    """Вернуть {track_id: TrackMetadata}. Пустой словарь, если что-то пошло не так."""
