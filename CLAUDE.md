...
- **Четыре таблицы**: `loops`, `tracks`, `renders`, `video_analyses`.
  ...
  baseline'ом, отдельного поля "способ подбора" не заводили. С итерации 5
  — `rating` (`good`/`neutral`/`bad`, CHECK-ограничение, nullable, см.
  migrations/0007) — оценка пользователя из UI.
...
- **Граф-агент** (итерация 5, [agent_graph.py](src/sound_loops/agent_graph.py)):
  ...
  - **Оценка превью пользователем** (`renders.rating`, migrations/0007,
    `good`/`neutral`/`bad`, кнопки под каждым вариантом в UI,
    `render.py::set_render_rating`) — свойство конкретного рендера
    (трек мог подойти одному лупу и не подойти другому), не трека. `bad`
    даёт постоянный эффект: `get_bad_rated_track_ids(conn, loop_id)`
    исключает такие треки из поиска для **того же лупа** во всех будущих
    сессиях (`plan_node` объединяет это с сессионным `rejected_track_ids`)
    — не глобально, трек, не подошедший одной сцене, может подойти другой.
